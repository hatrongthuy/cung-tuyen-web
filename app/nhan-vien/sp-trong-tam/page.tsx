...
export default async function SpTrongTamNhanVienPage({
  ...
  const sptt = buildSptt(data, range.nowFromMs, range.nowToMs, range.prevFromMs, range.prevToMs, onlyTid >= 0 ? onlyTid : -1);

  return (
    <>
      <AppHeader hoTen={user.name ?? ""} role="employee" active="sp-trong-tam" />
      <main className="mx-auto w-full max-w-6xl flex-1 px-4 py-6">
        <SpTrongTamView teamName={TEN_NHOM_HIEN_THI} sptt={sptt} range={range} mine />
      </main>
    </>
  );
}
